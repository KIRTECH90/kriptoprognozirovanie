...
import type { ModelPayload } from "@/lib/corridor/journal.ts";
...
export function ModelAudit({
  ...
  data: ModelPayload | null;
  busy: boolean;
  error: string | null;
}) {
  if (busy && !data) {
    return <p className="mt-6 text-sm text-muted">Прогоняю модель по истории этой пары…</p>;
  }
  ...
  return (
    <div className={cn("mt-6", busy && "opacity-60")}>
      <section className="rounded-[var(--radius-xl)] bg-surface p-5 shadow-[var(--shadow-border)]">
        <p className="text-xs font-medium uppercase tracking-widest text-subtle">Ретроспектива 24 часа</p>
        <p className="mt-2 font-mono text-4xl font-semibold tabular-nums tracking-tight">
          {cov == null ? "—" : `${Math.round(cov * 100)}%`}
        </p>
        <p className="mt-2 text-sm leading-relaxed text-fg">
          {n < 20
            ? `Пока мало суток (${n}). Рано говорить, врёт модель или нет.`
            : `Если бы коридор считали тогда: из ${data.nDays24} непересекающихся суток факт закрылся внутри ${Math.round((cov ?? 0) * n)} раз. Цель — ${Math.round(data.target24 * 100)}%.`}
        </p>
        <p className={cn("mt-2 text-sm", ok ? "text-ok" : weak ? "text-event" : "text-muted")}>
          {cov == null
            ? "Нет фактов."
            : ok
              ? "На истории похоже на заявленное покрытие."
              : weak
                ? "На истории хуже цели — коридор узкий или режим редкий."
                : "Около цели на прошлом, не на выданных прогнозах."}
        </p>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <p className="rounded-[var(--radius-sm)] bg-bg-elevated px-3 py-2.5">
            ...
            <span className="mt-0.5 block text-xs text-muted">{data.n48} суток</span>
          </p>
          <p className="rounded-[var(--radius-sm)] bg-bg-elevated px-3 py-2.5">
        ...
        <p className="mt-4 text-xs leading-relaxed text-subtle">
          Это диагностика модели, не журнал приложения. Живые попадания — во вкладке Журнал.
          История ~{Math.round(data.hours / 24)} дней часовиков.
          {data.empiricalReady
            ? " Длины хватает на квантили того же режима."
      ...
      <section className="mt-4 rounded-[var(--radius-lg)] bg-surface p-5 shadow-[var(--shadow-border)]">
        <h2 className="text-base font-semibold">Последние сутки</h2>
        <p className="mt-1 text-xs text-muted">Как если бы открыли приложение в тот день.</p>
          ...
          {data.rows.map((r) => (
            <li key={r.ts} className="border-t border-border pt-3 first:border-t-0 first:pt-0">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <p className={cn("text-xs font-medium", r.hit24 ? "text-ok" : "text-event")}>
                  {r.hit24 ? "попал" : "мимо"}
                </p>
              </div>
