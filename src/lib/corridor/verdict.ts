...
export function buildVerdict(opts: {
  ...
  if (waitEvent || waitStale || waitTop || waitFloor || waitFlat) {
    ...
    if (waitEvent) {
      holdHours = 12;
      reason = opts.event
        ? "Сейчас резкое движение. Уклона нет — рано делать вывод."
        : "Сильные новости ещё доигрываются. Лучше смотреть коридор, не точку.";
    } else if (waitTop) {
      holdHours = 12;
      reason = "Цена у верха месяца и RSI высокий. Уклон вверх здесь слабый.";
    } else if (waitFloor) {
      holdHours = 12;
      reason = "Цена уже у низа месяца. Уклон вниз здесь слабый.";
    } else if (waitStale) {
      holdHours = 12;
      reason = "Данных или уверенности не хватает. Смотрите коридор, не точку.";
    } else {
      reason = "Ориентир почти как сейчас. Это не сигнал — центр специально прижат к рынку.";
    }
    return {
      side: "wait",
      strength: "weak",
  ...
  const reason =
    side === "buy"
      ? `${align} Уклон вверх (${how}). Ориентир около ${px} (${formatPct(move)}) на ${holdHours} ч. Это не приказ купить.`
      : `${align} Уклон вниз (${how}). Ориентир около ${px} (${formatPct(move)}) на ${holdHours} ч. Это не приказ продать.`;

  return {
    side,
    strength,
    label: side === "buy" ? "Вверх" : "Вниз",
    reason,
    holdHours,
    target,
    invalidation,
